...
export class RouterErrorService {
    ...
    requestIsNotEligible(message: string) {
        return new RouterException('not_eligible', message, 201)
    }
    requestUserBlock(message: string) {
        return new RouterException('blocked', message, 509)
    }
}